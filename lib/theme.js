...
function _setFiletree(settings, theme) {
  ...
  theme = theme.replace(config.regex.filetree.fontSize, `$1${settings.filetree.font.size}$3`)
  return theme
}
...
function _setTabs(settings, theme) {
  ...
  theme = theme.replace(config.regex.tabs.fontSize, `$1${settings.tabs.font.size}$3`)
  return theme
}
...
function _setStatusbar(settings, theme, reset) {
  ...
  theme = theme.replace(config.regex.statusbar.fontSize, `$1${settings.statusbar.font.size}$3`)
  if (reset) {
