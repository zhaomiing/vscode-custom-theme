...
// @TODO test on fedora
// @TODO add support for light theme (need to extend default config)
...
function _setFiletree(settings, theme) {
  theme = theme.replace(config.regex.filetree.background, `$1${settings.filetree.background.color}$3`)
  theme = theme.replace(config.regex.filetree.toolbox, `$1${settings.filetree.background.color}$3`)
  return theme
}

function _setTabs(settings, theme) {
  theme = theme.replace(config.regex.tabs.background, `$1${settings.tabs.background.color}$3`)
  theme = theme.replace(config.regex.tabs.border.left, `$1${settings.tabs.border.color}$3`)
  theme = theme.replace(config.regex.tabs.border.right, `$1${settings.tabs.border.color}$3`)
  theme = theme.replace(config.regex.tabs.border.bottom, `$1${settings.tabs.border.color}$3`)
  theme = theme.replace(config.regex.tabs.toolbox.background, `$1${settings.tabs.toolbox.background.color}$3`)
  return theme
}
